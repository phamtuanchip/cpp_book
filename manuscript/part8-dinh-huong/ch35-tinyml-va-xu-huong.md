...
- **Requantize**: sau khi tích luỹ, kết quả phải quy đổi **trở lại** thang đo `int8` của tầng tiếp theo — ví dụ dùng `float` cho phép nhân với `scale` trong minh hoạ để dễ đọc; bản triển khai tối ưu thật (như TensorFlow Lite Micro) dùng **nhân số nguyên cố định + dịch bit** để tránh hoàn toàn `float`, kể cả ở bước này.
